...
export default function Analytics() {
  ...
  return (
    <div>
      <div className="mb-10">
        <h1 className="text-2xl font-semibold text-gray-950">{t('analytics.title')}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-7 mb-9">
        {[
          [t('analytics.platformRevenue'), formatCurrency(revenue)],
          [t('analytics.activeUsers'), activeUsers.toLocaleString("fr-FR")],
          [t('analytics.activeProducts'), activeProducts.toLocaleString("fr-FR")],
          [t('analytics.conversionRate'), `${conversion}%`],
        ].map(([label, value]) => (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-7 mb-9">
        <section className="bg-white rounded-3xl p-7 shadow-sm min-h-[450px]">
          <h2 className="text-xl font-semibold mb-7 text-gray-950">{t('analytics.salesGrowth')}</h2>
        ...
        <section className="bg-white rounded-3xl p-7 shadow-sm min-h-[450px]">
          <h2 className="text-xl font-semibold mb-7 text-gray-950">{t('analytics.salesByCategory')}</h2>
          {categoryData.length ? (
            <ResponsiveContainer width="100%" height={330}>
              <PieChart>
            ...
            <div className="h-[330px] flex items-center justify-center text-gray-500">{t('analytics.noCategoryData')}</div>
          )}
        </section>
      </div>

      <section className="bg-white rounded-3xl p-7 shadow-sm min-h-[500px]">
        <h2 className="text-xl font-semibold mb-7 text-gray-950">{t('analytics.topVendors')}</h2>
        {vendorData.length ? (
          ...
          <div className="h-[390px] flex items-center justify-center text-gray-500">{t('analytics.noVendorSales')}</div>
        )}
      </section>
    </div>
  );
